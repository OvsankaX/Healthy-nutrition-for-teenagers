...
app.post('/api/register', async (req, res) => {
    ...
    if (attempts && attempts.count >= 3) {
        return res.status(429).json({ error: 'TOO_MANY_ACCOUNTS', message: 'Не более 3 аккаунтов в час' });
    }
    
    if (username.length > 16 || username.length < 3) return res.status(400).json({ error: 'INVALID_USERNAME' });
    if (password.length > 32 || password.length < 3) return res.status(400).json({ error: 'INVALID_PASSWORD' });
    ...
    try {
        ...
        console.log(`📝 New user: ${username} from ${clientIp}`);
...
app.post('/api/login', async (req, res) => {
    ...
    try {
        ...
        if (!bcrypt.compareSync(password, user.password)) {
            console.log(`⚠️ Failed login: ${username} from ${clientIp}`);
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`🔐 Security: Rate limiting + PostgreSQL`);
});
